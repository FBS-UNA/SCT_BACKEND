...
INSERT INTO USUARIOS_ROLES VALUES ('VIGILANTE', '402450246');

INSERT INTO USUARIOS_ROLES VALUES ('RECEPCION', '208130058');
INSERT INTO USUARIOS_ROLES VALUES ('PRODUCTOSYSERVICIOS', '208130058');

DELETE FROM USUARIOS_ROLES WHERE CEDULA_USUARIO = '402530551' AND NOMBRE_ROL = 'ADMINISTRADOR';
...
/*Nombre de cada tramite y su cantidad realizada*/
SELECT T.NOMBRE_TRAMITE, COUNT(*) AS CANTIDAD
FROM REGISTRO_TRAMITE R
JOIN TRAMITES T ON R.ID_TRAMITE = T.ID_TRAMITE
GROUP BY T.ID_TRAMITE, T.NOMBRE_TRAMITE;

/*Nombre de cada AREA y la cantidad de tramites realizados en la misma*/
SELECT A.NOMBRE_AREA, COUNT(*) AS CANTIDAD
FROM REGISTRO_TRAMITE R
JOIN AREAS A ON R.ID_AREA = A.ID_AREA
GROUP BY A.ID_AREA, A.NOMBRE_AREA;

/*Cantidad total de reportes hasta la fecha*/
SELECT COUNT(*) AS CANTIDAD_REGISTROS FROM REGISTRO_TRAMITE;


/*Cantidad de reportes por mes*/
SELECT TO_CHAR(FECHA_Y_HORA, 'MONTH') AS MES, COUNT(*) AS CANTIDAD FROM REGISTRO_TRAMITE RT GROUP BY TO_CHAR(FECHA_Y_HORA, 'MONTH');
